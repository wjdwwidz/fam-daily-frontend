  ...
  return (
    <Pressable onPress={vm.closeQuestion} style={s('position:absolute;inset:0;background:rgba(23,48,59,0.5);z-index:45;align-items:flex-end;animation:sfade .18s ease')}>
      <Pressable onPress={vm.stopEvt} style={s('width:100%;background:#fff;border-radius:26px 26px 0 0;padding:20px 20px 28px;animation:sheetup .26s cubic-bezier(.4,0,.2,1)')}>
        ...
        <Text style={s('font-size:16px;font-weight:800;color:#17303B;margin-top:6px;line-height:1.4')}>가족에게 물어보고 싶은 질문은?</Text>
        <TextInput
          multiline
          ...
          style={s('width:100%;min-height:88px;margin-top:16px;border:1px solid #FFE1EC;outline:none;background:#FFF6FB;border-radius:14px;padding:14px 15px;font-size:13.5px;font-family:inherit;color:#17303B;resize:none')}
        />
        <Pressable onPress={vm.fillSuggestedQuestion} style={s('margin-top:12px;align-self:flex-start;flex-direction:row;align-items:center;gap:6px;background:#FCEEF4;border-radius:12px;padding:9px 13px')}>
          <Text style={s('font-size:12px')}>🎲</Text>
          <Text style={s('font-size:12px;font-weight:700;color:#FF5E8A')}>추천 질문 받기</Text>
        </Pressable>
        <Pressable onPress={vm.submitQuestion} style={s('margin-top:18px;height:54px;border-radius:17px;background:#FF5E8A;align-items:center;justify-content:center;cursor:pointer')}>
