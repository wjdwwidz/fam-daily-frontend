  ...
  return (
    <Pressable onPress={vm.closeInvite} style={s('position:absolute;inset:0;background:rgba(23,48,59,0.5);z-index:45;display:flex;align-items:flex-end;animation:sfade .18s ease')}>
      <Pressable onPress={vm.stopEvt} style={s('width:100%;background:#fff;border-radius:26px 26px 0 0;padding:20px 20px 30px;animation:sheetup .26s cubic-bezier(.4,0,.2,1)')}>
        ...
        <Text style={s('font-size:11.5px;color:#9DB2BD;margin-top:5px;margin-bottom:16px')}>참여 코드를 공유하면 가족이 앱에서 입력해 들어와요</Text>

        {vm.inviteLoading ? (
          <Text style={s('padding:18px;text-align:center;color:#9DB2BD;font-size:12.5px')}>초대 코드 생성 중…</Text>
        ) : vm.inviteError ? (
          <Text style={s('padding:14px;text-align:center;color:#E5484D;font-size:12.5px')}>{vm.inviteError}</Text>
        ) : vm.inviteCode ? (
          <View style={s('background:#FFF6FB;border:1.5px dashed #FFC4D8;border-radius:14px;padding:14px;margin-bottom:16px;align-items:center')}>
            <Text style={s('font-size:11px;color:#9DB2BD;margin-bottom:5px')}>참여 코드</Text>
            <Text style={s('font-size:23px;font-weight:800;color:#FF5E8A;letter-spacing:2px')}>{vm.inviteCode}</Text>
          </View>
        ) : null}

        <Pressable onPress={vm.shareInvite} disabled={!vm.inviteCode} style={s(`display:flex;align-items:center;gap:12px;background:#FEE500;border-radius:16px;padding:15px 16px;cursor:pointer;opacity:${vm.inviteCode ? 1 : 0.5}`)}>
          ...
          <View style={s('flex:1')}><Text style={s('font-size:13.5px;font-weight:800;color:#3C1E1E')}>카카오톡·메시지로 공유</Text><Text style={s('font-size:11px;color:#7A5C2E;margin-top:1px')}>공유 시트에서 카카오톡 선택</Text></View>
        ...
        <Pressable onPress={vm.copyInvite} disabled={!vm.inviteCode} style={s(`display:flex;align-items:center;gap:12px;background:#FFF6FB;border:1px solid #FFE1EC;border-radius:16px;padding:15px 16px;margin-top:10px;cursor:pointer;opacity:${vm.inviteCode ? 1 : 0.5}`)}>
          ...
          <View style={s('flex:1')}><Text style={s('font-size:13.5px;font-weight:800;color:#17303B')}>{vm.inviteCopied ? '복사됨! ✓' : '초대 코드 복사'}</Text><Text style={s('font-size:11px;color:#9DB2BD;margin-top:1px')}>코드·링크를 클립보드에 복사</Text></View>
