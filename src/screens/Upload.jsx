...
export default function Upload({ vm }) {
  return (
    <View style={s('padding:0 0 40px')}>
      ...
      <View style={s('padding:6px 20px 0')}>
        <View style={s('margin-top:16px;height:200px;border-radius:28px;border:2px dashed #BEE6F7;background:#F3FAFE;flex-direction:column;align-items:center;justify-content:center;gap:12px')}>
          ...
          <View style={s('text-align:center')}>
            ...
            <Text style={s('font-size:11px;color:#9DB2BD;margin-top:3px')}>갤러리에서 선택하거나 촬영하세요</Text>
          </View>
        </View>
        <Text style={s('margin-top:18px;font-size:11.3px;font-weight:700;color:#17303B;margin-bottom:8px')}>설명</Text>
        <TextInput multiline textAlignVertical="top" placeholder="이 순간을 한 줄로 남겨보세요" placeholderTextColor="#9DB2BD" style={s('width:100%;min-height:64px;border:none;outline:none;background:#fff;border:1px solid #FFE1EC;box-shadow:0 10px 24px rgba(255,94,138,0.13);border-radius:16px;padding:14px 15px;font-size:12.2px;font-family:inherit;color:#17303B;resize:none')} />
        <Text style={s('margin-top:18px;font-size:11.3px;font-weight:700;color:#17303B;margin-bottom:10px')}>함께한 가족 태그</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s('gap:12px;padding-bottom:4px')}>
          {vm.members.map((m, i) => (
            <View key={i} style={s('display:flex;flex-direction:column;align-items:center;gap:5px;flex:0 0 auto')}>
        ...
        <Pressable onPress={vm.goHome} style={s('margin-top:22px;height:54px;border-radius:17px;background:#FF5E8A;align-items:center;justify-content:center;flex-direction:row')}>
