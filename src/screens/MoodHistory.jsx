    ...
    <View style={s('padding:8px 20px 110px')}>
      ...
      <View>
        {vm.moodHistory.map((g, i) => (
          <View key={i}>
            <View style={s('display:flex;align-items:center;gap:10px;margin:14px 2px 4px')}>
            ...
            {g.items.map((h, j) => (
              <View key={j} style={s('display:flex;align-items:flex-start;gap:11px;padding:9px 2px;border-bottom:1px solid #F3EAF0')}>
                ...
                <View style={s('flex:1;min-width:0')}>
                  <View style={s('flex-direction:row;align-items:baseline;gap:6px')}>
                    <Text style={s('font-size:12px;color:#17303B;font-weight:700')}>{h.name}</Text>
                    <Text style={s('font-size:10px;color:#C0B3BB')}>{h.when}</Text>
                  </View>
                  <Text style={s('font-size:12.5px;color:#3A4A54;margin-top:3px;line-height:1.5')}>{h.text}</Text>
                </View>
              </View>
            ))}
          </View>
