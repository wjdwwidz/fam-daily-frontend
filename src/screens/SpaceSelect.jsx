...
export default function SpaceSelect() {
  ...
  return (
    <View style={s('flex:1;padding:16px 24px 24px')}>
      <Pressable onPress={vm.logout} style={s('width:40px;height:40px;border-radius:13px;background:#fff;border:1px solid #FFE1EC;box-shadow:0 10px 24px rgba(255,94,138,0.13);align-items:center;justify-content:center;margin-bottom:14px')}>
      ...
      <Text style={s('font-size:14px;color:#6A7E88;margin-top:6px;margin-bottom:24px')}>여러 가족 공간에 참여할 수 있어요</Text>

      {vm.groupsLoading && (
        <View style={s('align-items:center;padding:30px')}>
          <ActivityIndicator color="#FF5E8A" />
          <Text style={s('font-size:13px;color:#9DB2BD;margin-top:10px')}>가족 공간 불러오는 중…</Text>
        </View>
      )}

      {empty && (
        <View style={s('align-items:center;padding:24px 10px 30px')}>
      ...
      <View style={s('flex-direction:column;gap:10px;margin-top:4px')}>
        <Pressable onPress={vm.goCreate} style={s('flex-direction:row;align-items:center;justify-content:center;gap:9px;border:1.5px dashed #FFC4D8;border-radius:20px;padding:17px;background:#FFF6FA')}>
        ...
        <Pressable onPress={vm.goJoin} style={s('flex-direction:row;align-items:center;justify-content:center;gap:9px;border:1.5px dashed #C7E7F7;border-radius:20px;padding:17px;background:#F3FAFE')}>
