  ...
  return (
    <View style={s('flex:1;align-items:center;justify-content:center;padding:40px 32px;background:#FFF6FB;position:relative;overflow:hidden')}>
      ...
      <View style={s('width:100%;margin-top:44px;flex-direction:column;gap:12px')}>
        <Pressable onPress={vm.kakaoLogin} disabled={vm.authLoading} style={s(`height:56px;border-radius:24px;background:#FEE500;flex-direction:row;align-items:center;justify-content:center;gap:9px;opacity:${vm.authLoading ? 0.7 : 1}`)}>
      ...
      {vm.authLoading && <View style={s('margin-top:16px')}><ActivityIndicator color="#FF5E8A" /></View>}
      {vm.authError && <Text style={s('font-size:12px;color:#E5484D;margin-top:14px;text-align:center;line-height:1.5')}>{vm.authError}</Text>}

      <Text style={s('font-size:11.5px;color:#9DB2BD;margin-top:26px;line-height:1.5;text-align:center')}>
