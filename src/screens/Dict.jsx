  ...
  return (
    <View style={s('padding:md 5xl 110px')}>
      ...
      {vm.dictGroups.map((grp, i) => (
        <View key={i}>
          <View style={s('display:flex;align-items:center;gap:lg;margin:4xl hair lg')}>
          ...
          <View style={s('background:#fff;border:1px solid #F3DCE4;border-radius:2px;overflow:hidden')}>
            {grp.items.map((w, j) => (
              <Pressable key={j} onPress={w.open} style={s('display:flex;align-items:center;gap:xl;padding:lg 3xl;cursor:pointer;border-bottom:1px solid #FDECF2')}>
